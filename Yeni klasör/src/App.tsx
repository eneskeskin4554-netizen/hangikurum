...
import { Search, Filter, ArrowUpDown, Globe, Building2, Mail, Sparkles, TrendingUp, Bitcoin, CalendarCheck, Percent, Star, Flame } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const applyQuickFilter = (type: 'commission' | 'score' | 'popular') => {
    if (type === 'commission') {
      setSearchTerm('');
      setSortKey('commissionRate');
      setSortOrder('asc');
    } else if (type === 'score') {
      setSearchTerm('');
      setSortKey('mobileAppScore');
      setSortOrder('desc');
    } else if (type === 'popular') {
      setSearchTerm('Popüler');
      setSortKey('mobileAppScore');
      setSortOrder('desc');
    }
  };
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
              <div>
                ...
                {/* Search Bar */}
                <div className="relative max-w-md mb-4 group animate-fade-in delay-300">
                ...
                {/* Quick Filters */}
                <div className="flex flex-wrap gap-2 max-w-md animate-fade-in delay-300 mb-8">
                  <button 
                    onClick={() => applyQuickFilter('commission')}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white border border-slate-200 text-xs font-semibold text-slate-600 hover:border-blue-300 hover:text-blue-600 hover:bg-blue-50 transition-all shadow-sm"
                  >
                    <Percent size={14} /> En Düşük Komisyon
                  </button>
                  <button 
                    onClick={() => applyQuickFilter('score')}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white border border-slate-200 text-xs font-semibold text-slate-600 hover:border-yellow-300 hover:text-yellow-600 hover:bg-yellow-50 transition-all shadow-sm"
                  >
                    <Star size={14} /> En Yüksek Puan
                  </button>
                  <button 
                    onClick={() => applyQuickFilter('popular')}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white border border-slate-200 text-xs font-semibold text-slate-600 hover:border-red-300 hover:text-red-600 hover:bg-red-50 transition-all shadow-sm"
                  >
